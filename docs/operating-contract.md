...
- `MEMOREPO_AGENT_PROVIDER_ID` and `MEMOREPO_AGENT_MODEL_ID`: the initial Ask this Space selection. They are read from the environment; `.env.example` supplies `openai-codex` and `gpt-5.4`. Dashboard changes to the global provider, model, verbosity, and reasoning-effort selection are persisted in SQLite and restored after API restarts. If the saved selection is absent from the current catalog, MemoRepo falls back to the configured initial selection.
- `MEMOREPO_AGENT_MAX_RUN_SECONDS`: the per-attempt wall-clock safety ceiling, defaulting to `1800`.
- `MEMOREPO_AGENT_MAX_TOOL_CALLS`: the per-attempt tool-call safety ceiling, defaulting to `200`.
- `MEMOREPO_AGENT_MAX_PROVIDER_ROUNDS`: the per-attempt provider-round safety ceiling, defaulting to `50`.
...
- `MEMOREPO_CBM_INDEX_MODE`: the CBM indexing depth used by immutable snapshots and legacy repository indexes. Accepted values are `fast`, `moderate`, and `full`; the default is `fast`. Invalid values stop API startup. Changing the mode requires rebuilding snapshots before retained chats can use the new index capabilities.
...
The space-level check and update job fetches each selected remote branch and compares its remote commit with the commit recorded by MemoRepo. In snapshot-only mode, it skips repositories whose commit is unchanged even when their unused mutable-index state is stale. Legacy mutable-index mode repairs stale or failed indexes when the commit is unchanged. The job builds one replacement snapshot only when repository or snapshot state requires it. Per-repository reindex remains an explicit forced rebuild operation.
...
Each snapshot-only batch creates one clone and checkout chain per repository, exactly one primary snapshot index per repository, and one coalesced snapshot rebuild. Selection validation, space membership, batch metadata, jobs, and dependencies are committed in one SQLite transaction, so an invalid repository or enqueue failure leaves no partial batch. The client supplies a space-scoped request ID: repeating the same request returns the original batch, while reusing that ID for a different repository set is rejected. Batch status groups repository preparation and shared-snapshot progress; cancellation targets every active job in the batch, and retry creates only the recovery work required by the current repository state. The snapshot activates only after every repository dependency succeeds.

Managed clones first request Git's blobless partial-clone filter and omit tags. Servers that explicitly reject filtering fall back to a normal no-tags clone; checkout still materializes the selected commit before immutable snapshot indexing. Run `pnpm --filter @memorepo/api perf:ingestion` to compare sequential-after-terminal and batch plans for 1, 3, and 5 repositories under identical scheduler settings. The command fails unless batch ingestion produces exactly one rebuild, one primary index per repository, and at least a 60% workflow reduction for five repositories. Use `pnpm perf:baseline -- --repositories owner/one,owner/two,owner/three --ingestion-mode batch` to measure the real batch API against a running MemoRepo instance.
...
Visible agent messages, source references, chat metadata, provider account-session identifiers, selected provider/model settings, adaptive execution policy, recovery attempts, queue state, and aggregate turn diagnostics are stored in SQLite. Turn diagnostics include completion reason, answer quality, final stop reason, provider round and length-stop counts, tool-call count, and input, output, reasoning, cache, and total token usage. Each attempt may additionally retain a bounded failure category and stage, allowlisted provider code and request identifiers, HTTP status, transport, retryability, bounded retry delay, total duration, and time to first provider event. Diagnostic summaries are generated locally from the classification. Failure diagnostics and logs do not retain additional prompt copies, raw provider errors or response bodies, arbitrary headers, or model reasoning. Successful bounded read-only tool results are stored in a size-limited cache keyed by immutable snapshot, tool, and canonical arguments so a resumed attempt can reuse evidence without repeating the query. Cache rows are removed when their snapshot is deleted. Managed agent OAuth credentials are stored separately in an owner-only file under `MEMOREPO_SECRETS_DIR`; Docker Compose persists that file in the existing private `memorepo-secrets` volume.

Answers run through a configurable-capacity FIFO dispatcher. Waiting and running answers survive an API restart and resume when the matching provider connection is available. Queue position and aggregate capacity are part of the API contract. Queued and running answers can be cancelled. Provider failures classified as transient or unknown recover automatically for up to three total attempts, with bounded exponential and provider-requested delays; explicitly non-retryable failures stop after the current attempt. The dashboard reports the active automatic attempt. An exhausted failure or interruption can be resumed on the same logical turn, and a completed best-effort answer can continue investigating. Recovery preserves the original user and assistant message identifiers. A queue-full response is returned only after the configured waiting capacity is exhausted.
